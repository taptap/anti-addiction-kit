 ...
 */
public final class Options extends AbstractList<com.tapsdk.antiaddiction.skynet.okio.ByteString> implements RandomAccess {
    final com.tapsdk.antiaddiction.skynet.okio.ByteString[] byteStrings;
    final int[] trie;

    private Options(com.tapsdk.antiaddiction.skynet.okio.ByteString[] byteStrings, int[] trie) {
        this.byteStrings = byteStrings;
        this.trie = trie;
    }

    public static Options of(com.tapsdk.antiaddiction.skynet.okio.ByteString... byteStrings) {
        if (byteStrings.length == 0) {
            // With no choices we must always return -1. Create a trie that selects from an empty set.
            return new Options(new com.tapsdk.antiaddiction.skynet.okio.ByteString[0], new int[]{0, -1});
        }

        // Sort the byte strings which is required when recursively building the trie. Map the sorted
        // indexes to the caller's indexes.
        List<com.tapsdk.antiaddiction.skynet.okio.ByteString> list = new ArrayList<>(Arrays.asList(byteStrings));
        ...
        for (int a = 0; a < list.size(); a++) {
            com.tapsdk.antiaddiction.skynet.okio.ByteString prefix = list.get(a);
            for (int b = a + 1; b < list.size(); ) {
                com.tapsdk.antiaddiction.skynet.okio.ByteString byteString = list.get(b);
        ...
        com.tapsdk.antiaddiction.skynet.okio.Buffer trieBytes = new com.tapsdk.antiaddiction.skynet.okio.Buffer();
    ...
    private static void buildTrieRecursive(
            long nodeOffset,
            com.tapsdk.antiaddiction.skynet.okio.Buffer node,
            int byteStringOffset,
            List<com.tapsdk.antiaddiction.skynet.okio.ByteString> byteStrings,
            int fromIndex,
            int toIndex,
        ...
        com.tapsdk.antiaddiction.skynet.okio.ByteString from = byteStrings.get(fromIndex);
        com.tapsdk.antiaddiction.skynet.okio.ByteString to = byteStrings.get(toIndex - 1);
        int prefixIndex = -1;
        ...
        if (from.getByte(byteStringOffset) != to.getByte(byteStringOffset)) {
            ...
            com.tapsdk.antiaddiction.skynet.okio.Buffer childNodes = new com.tapsdk.antiaddiction.skynet.okio.Buffer();
